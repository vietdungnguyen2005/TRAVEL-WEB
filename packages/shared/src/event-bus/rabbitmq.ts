...
import { ensureCorrelationId, runWithCorrelationId } from '../observability/correlation';
 ...
 */
const ACK_HANDLED: unique symbol = Symbol('rabbit-ack-handled');
type AckableMessage = ConsumeMessage & { [ACK_HANDLED]?: boolean };
...
export type ConsumeWithRetryOptions = ConsumeOptions & {
    ...
    // Optional hook for observability (e.g., Prometheus counters) when a retry is scheduled.
    ...
    await ch.consume(
        ...
        async (msg) => {
            ...
            const correlationId = ensureCorrelationId(
                (typeof msg.properties.correlationId === 'string' && msg.properties.correlationId.trim())
                    ? msg.properties.correlationId
                    : ((msg.properties.headers as Record<string, unknown> | undefined)?.['x-correlation-id'] as string | undefined),
            );
            ...
            await runWithCorrelationId(correlationId, async () => {
                try {
                    ...
                    if (!(msg as AckableMessage)[ACK_HANDLED]) {
                        ch.ack(msg);
                    }
                } catch (err) {
                    // Skip nack if the handler already acked/nacked
                    if ((msg as AckableMessage)[ACK_HANDLED]) return;
    ...
    await rabbitConsume(
        ...
        async (payload, raw) => {
            try {
                ...
                (raw as AckableMessage)[ACK_HANDLED] = true;
                ch.ack(raw);
                return;
            }
        },
