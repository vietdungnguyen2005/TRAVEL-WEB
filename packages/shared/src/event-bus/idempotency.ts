type PrismaWithProcessedEvent = {
    processedEvent: {
        ...
        deleteMany: (args: { where: { consumer: string; messageId: string } }) => Promise<unknown>;
    };
};

function tryGetErrorCode(err: unknown): string | undefined {
...
export async function withEventIdempotency<T>(
    prisma: PrismaWithProcessedEvent,
    params: {
        consumer: string;
        messageId: string;
        eventType?: string;
