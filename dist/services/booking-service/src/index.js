...
app.listen(PORT, () => {
    ...
    publishOutbox().catch((err) => logger.error('Outbox publisher failed to start', err));
    // start consumer for payment.* events
    startPaymentEventsConsumer().catch((err) => logger.error('Payment events consumer failed to start', err));
