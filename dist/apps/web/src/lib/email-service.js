...
export async function sendCheckInReminderEmail(booking) {
    try {
        const emailHtml = await render(CheckInReminderEmail({
            customerName: booking.user.name || 'Khách hàng',
            bookingId: booking.id.slice(0, 8).toUpperCase(),
            roomType: booking.room.roomType.name,
            roomNumber: booking.room.roomNumber,
...
export async function sendCancellationEmail(booking, refundAmount, cancellationReason) {
    try {
        const emailHtml = await render(CancellationEmail({
            customerName: booking.user.name || 'Khách hàng',
            bookingId: booking.id.slice(0, 8).toUpperCase(),
            roomType: booking.room.roomType.name,
            checkInDate: format(new Date(booking.checkIn), 'dd/MM/yyyy', { locale: vi }),
